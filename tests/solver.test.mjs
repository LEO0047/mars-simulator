...
test('Lambert solver at the Hohmann window reproduces a Hohmann-like transfer', () => {
    const launchDay = Math.round(calculateOptimalLaunchDay(TRANSFER_MODES.hohmann));
    const solution = solveTransferForLaunchDay(launchDay);
    assert.ok(solution, 'expected a solution at the Hohmann window');
    assert.ok(Math.abs(solution.durationDays - 259) < 15, `duration ${solution.durationDays}`);
    assert.ok(Math.abs(solution.eccentricity - TRANSFER_MODES.hohmann.eccentricity) < 0.05);
...
test('Lambert solver finds an intercepting transfer for every sampled launch day', () => {
    ...
    for (let i = 0; i < 12; i += 1) {
        ...
        const earth = getPlanetPosition(
            CONSTANTS.EARTH_ORBIT_AU, CONSTANTS.EARTH_PERIOD_DAYS, launchDay);
...
test('solved transfers start on Earth orbit and end on Mars orbit', () => {
    for (const launchDay of [0, 200, 500, 684]) {
        const solution = solveTransferForLaunchDay(launchDay);
        assert.ok(solution, `no solution on day ${launchDay}`);
        assert.ok(Math.abs(getTransferPosition(solution, 0, 0).radiusAU - 1) < 0.01);
        assert.ok(
            Math.abs(getTransferPosition(solution, 1, 0).radiusAU - CONSTANTS.MARS_ORBIT_AU) < 0.05);
    }
});

test('off-window launches cost more delta-v than the Hohmann window', () => {
    const windowDay = Math.round(calculateOptimalLaunchDay(TRANSFER_MODES.hohmann));
    const atWindow = solveTransferForLaunchDay(windowDay);
    const offWindow = solveTransferForLaunchDay(Math.round(windowDay + getSynodicPeriod() / 2) % 780);
    ...
    assert.ok(offTotal > windowTotal + 1, `expected clear penalty: ${windowTotal} vs ${offTotal}`);
});

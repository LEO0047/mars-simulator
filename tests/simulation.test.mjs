...
test('calculated Hohmann window reproduces the known launch epoch', () => {
    assert.ok(
        Math.abs(calculateOptimalLaunchDay(TRANSFER_MODES.hohmann) - 684) < 1,
    );
});
...
test('a deliberately late Hohmann launch misses Mars', () => {
    ...
    assert.equal(
        getLaunchGeometry(TRANSFER_MODES.hohmann, launchDay).rating,
        'miss',
    );
});
...
test('transfer endpoints begin at Earth orbit and finish at Mars orbit', () => {
    for (const mode of Object.values(TRANSFER_MODES)) {
        assert.ok(
            Math.abs(getTransferPosition(mode, 0, 0).radiusAU - 1) < 0.001,
        );
        assert.ok(
            Math.abs(
                getTransferPosition(mode, 1, 0).radiusAU -
...
test('fast-transfer delta-v is derived from the same vector formula as Hohmann', () => {
    ...
    assert.ok(
        getModeDeltaV(TRANSFER_MODES.fast) >
            getModeDeltaV(TRANSFER_MODES.hohmann),
    );
});

test('equal time intervals sweep equal areas on the transfer ellipse', () => {
    const mode = TRANSFER_MODES.hohmann;
    function area(start, end) {
        let sum = 0,
            prev = getTransferPosition(mode, start, 0);
        for (let i = 1; i <= 1000; i++) {
            const p = getTransferPosition(
                mode,
                start + ((end - start) * i) / 1000,
                0,
            );
            sum += (prev.x * p.y - prev.y * p.x) / 2;
            prev = p;
        }
        return sum;
    }
    const near = area(0, 0.25),
        far = area(0.75, 1);
    assert.ok(Math.abs(near - far) / near < 0.00001);
    const start = getTransferPosition(mode, 0.01, 0),
        end = getTransferPosition(mode, 0.99, 0);
    assert.ok(
        Math.hypot(start.x - 1, start.y) > Math.hypot(end.x + 1.524, end.y),
    );
});
